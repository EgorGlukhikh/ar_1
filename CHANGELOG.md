...
## 2026-03-22 (авторизация + роли)

### fix: стартовая страница — /login вместо лендинга
- `app/page.tsx`: неавторизованные → `/login`, авторизованные → кабинет по роли
- Лендинг доступен по `/landing` (для маркетинга/публичного просмотра)
- `signOut` теперь возвращает на `/login`

### feat: role switcher в navbar
- Для ADMIN в дропдауне аватара — блок «Просмотр как»:
  ⚙️ Админ / ✍️ Автор / 🎓 Куратор / 👤 Студент
- Активная роль подсвечена фиолетовым + галочка
- При переключении — `POST /api/admin/preview-role`, редирект на нужный дашборд
- Баннер (amber) в шапке при активном preview с кнопкой «Выйти из режима»
- Все ссылки navbar адаптируются под текущую активную роль
